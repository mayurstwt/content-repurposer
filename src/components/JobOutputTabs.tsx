        ...
        <div className="space-y-4 pt-3">
            ...
            {(data.suggested_clip_start || data.suggested_clip_end) && (
                <div className="flex gap-4 text-xs text-muted-foreground">
                    {data.suggested_clip_start && <span>⏱ Start: {data.suggested_clip_start}</span>}
...
function TwitterTab({ data }: { data: any }) {
    ...
    return (
        <div className="space-y-4 pt-3">
            <div className="space-y-1">
                <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Thread ({tweets.length} tweets)</span>
                ...
                <ol className="space-y-2">
                    {tweets.map((tweet: string, i: number) => (
                        <li key={i} className="text-sm bg-muted rounded-md p-3 flex gap-2">
                            ...
                            <span className="whitespace-pre-wrap leading-relaxed">{tweet}</span>
                        </li>
                    ))}
                </ol>
            </div>
...
export default function JobOutputTabs({ outputs }: Props) {
    if (!outputs) return null;

    return (
        <Tabs defaultValue="tiktok" className="mt-4">
